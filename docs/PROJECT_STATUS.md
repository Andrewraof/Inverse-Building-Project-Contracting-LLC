...
- 39 اختبارًا في `tests/test_meta_health.py` تغطي دلالات
  الاستلام (توقيع سيئ/حساب خاطئ/استيراد تاريخي/فشل معالجة/echo/فشل
  enqueue/فشل تسجيل رسالة)، الصمت الاختياري، عزل التراكم بين الحسابات
  ...
  **اختبار تزامن حقيقي بمعاملتين وcursors خامًا** على مفتاح التنبيه
  ...
  Check Now بصلاحية المدير).
- إصلاح مراجعة: مسار leadgen يختم الاستلام قبل الـenqueue (كمسار
  messaging)؛ فشل الـenqueue يُسجَّل منقّى ويجيب الـwebhook 200
  (Recovery Polling يغطي الليد) بدل 500.
- CI بمهمتين: الاختبارات القائمة + مهمة ترقية حقيقية
  ...
  الحالة: CI أخضر على الفرع — run 36232350433 على `333b58c`:
  `odoo-tests` ‏(0 فشل، 0 خطأ من 258 اختبارًا) و`odoo-upgrade-test`

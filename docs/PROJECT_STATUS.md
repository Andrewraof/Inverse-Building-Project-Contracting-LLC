...
- 33 اختبارًا جديدًا في `tests/test_meta_health.py` تغطي دلالات
  الاستلام (توقيع سيئ/حساب خاطئ/استيراد تاريخي/فشل معالجة/echo)،
  الصمت الاختياري، عزل التراكم بين الحسابات والشركات، دورة حياة
  التنبيه كاملة مع حماية الأنشطة غير المرتبطة، مسار IntegrityError
  تسلسليًا، حدود انتهاء التوكن والفحص القديم، الصفحات المؤرشفة،
  التشخيصات، والوصول (مدير/مستخدم/شركات) وعدم تسرب الأسرار.
  **لم تُشغَّل على بيئة Odoo فعلية بعد** — تعمل في CI على الفرع.

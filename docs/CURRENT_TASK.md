# CURRENT_TASK — Batch 3A: Meta Connector Health

الحالة: التنفيذ على الفرع المنفصل `batch3a-health-impl` للمراجعة فقط؛ لا
دمج في `main` ولا نشر إنتاج في هذه الخطوة. الإصدار `19.0.4.2.0` يضيف
مؤشرات استقبال الـwebhook الموقّع، نتيجة المعالجة، تشخيصًا صادقًا، شاشة
صحة محلية للمسؤولين وتنبيهات داخلية اختيارية. المراقبة معطّلة افتراضيًا
على كل حساب حتى يفعّلها المسؤول ويحدد مالكًا صالحًا.

المصدر التصميمي: `docs/BATCH3_HEALTH_DESIGN.md`؛ خطة التنفيذ:
`docs/superpowers/plans/2026-09-26-meta-health-alerts.md`. اختبارات Odoo
تعمل على قاعدة مؤقتة في GitHub Actions، وليس على `inverse_elite`. السحب
التاريخي لا يُحوَّل إلى دليل استقبال حي، ولا تُرسل الردود تلقائيًا من
التنبيهات. يلزم طلب منفصل للموافقة على النشر، ثم فحص إنتاج read-only
قبل تمكين التنبيهات واختبار حدث حي.

## السجل السابق — Batch 2
